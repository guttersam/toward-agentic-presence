...
Issues are ordered by build milestone. Do not work ahead of milestone dependencies.
...
- [ ] **#8** Convert retrieved memories into retrieve events — retrieved MemoryItems must become Event objects before entering the activation map; retrieval must not bypass the ledger
...
- [ ] **#15** Implement role anchor ingestion — chunk and embed role/SOUL.md; store chunks as role_anchor nodes in the graph with sub-centroid labels
...
- [ ] **#24** Add drive ablation switch — ablations.drive_modulation in settings.yaml should disable drive bias and drive-level changes; verify ablation affects candidate selection
...
- [ ] **#38** Write phase1_protocol.md — step-by-step instructions for running one complete Phase 1 cycle from cold start to validation export
- [ ] **#39** Write glossary.md — define: event file, feature canonicalization, associative activation, synthetic interoception, drive, workspace arbitration, recursive representational settlement, transmission gate, default mode, identity coherence, role anchor, ablation
...
These should be filed as future issues only after Phase 1 validation report is complete:
