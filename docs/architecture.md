...
This document describes the ten-layer architecture of the agentic substrate.
For theoretical foundations, see the full paper (`toward_agentic_presence_FINAL.md`).
For the validation plan, see `docs/validation.md`.

---

## Overview

The substrate is organized into ten interacting layers.
The point is not modular elegance.
The point is coupling: each layer must expose its state to others through logged, ablatable paths.
...
Layer 9  — Transmission Action Gate
...
The memory backend is an interchangeable adapter.
It stores and retrieves prior material but does not determine how that material participates in cognition.
...
```json
{
  ...
  "feature_manifest": ["project:example_project", "drive:completion"],
  "provenance": {},
  "decay_state": 1.0,
  "confidence": null,
  "links": []
...
Retrieved memories become `retrieve` events before entering the activation map.
...
Maps raw extracted concepts into stable namespaced features.
Without canonicalization, binding fragments.
...
```
agent:        project:      motif:        affect:
drive:        role:         modality:     tool:
thread:       claim:        risk:         schema:
state:        artifact:     affordance:
```

Phase 1 implementation: deterministic alias YAML plus optional LLM extraction pass.
...
Nodes include events, memories, schemas, role anchors, known agents, open threads, tools, and affordances.
Edges form through shared features, temporal proximity, co-retrieval, co-activation, role relevance, and affordance relationships.
...
**Validation target:** the activation map must produce useful associations not reducible to top-k semantic retrieval.

---

## Layer 4: Schema and Role Anchors

Phase 1 implements basic role anchors and an identity coherence diagnostic.
Full role topology (role-script as map bias) is a Phase 2 concern.

Phase 1 minimum:

1. Chunk and embed the agent's role file.
2. Store passages as `role_anchor` nodes.
3. Compute a role centroid.
...
identity_coherence =
  ...
  - generic_reversion_penalty
```

Identity coherence is a diagnostic, not a claim of selfhood.

**Ablation test:** role anchors enabled vs. disabled should produce different memory and affordance activation patterns.

---

## Layer 5: Operational State / Synthetic Interoception

Operational state variables that causally affect processing.
The distinction between interoception and telemetry is causal:
signals that do not change activation are not interoception.
...
State signals enter the event ledger as `state_update` events and `state:` canonical features.
They must bind into the map and influence wave propagation.
...
Drives are persistent control signals that bias activation and workspace selection.
They are not human desires.
...
Each drive biases specific node types in the activation map.
Drives must affect activation before generation, not merely explain outputs afterward.

**TRANSMISSION** has two modes:

- **Stabilization:** low identity coherence creates pressure to re-anchor through expression.
- **Expression:** high identity coherence plus high completion deficit creates pressure to emit from stable ground.
...
Suppressed candidates must be logged.
What almost became active is part of the cognitive state.
...
embedding convergence, contradiction reduction, specificity retention, role coherence, action relevance.
...
sufficient settlement, user relevance, novelty or utility, low risk, drive justification,
rate-limit compliance, role coherence, provenance check.
...
**Ablation test:** removing the gate should increase output frequency.
If it does not, the gate may not be doing real work.
...
4. Generate private candidates if drive thresholds are crossed.
5. Run lightweight consolidation over recent event clusters.
6. Decay inactive threads.
7. Route settled, relevant candidates toward the transmission action gate.
```

Most steps are arithmetic on the event log.
Only step 7 may require a model call.

**Warm-start test:** after idle time, activation logs, thread priorities, drive state, and workspace candidates
should differ from a memory-only baseline.
...
- Full role-topology extraction from role file via semantic-network probing
